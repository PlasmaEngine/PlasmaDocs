...
This component also enables a state machine to send messages to [components](components.md) attached to the same [game object](game-objects.md). For example the `plMsgStateMachineStateChanged` [event message](world-messaging.md#event-messages) will be broadcast on this object, if the state machine contains a corresponding state.
