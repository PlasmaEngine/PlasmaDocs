...
In general, visual scripting is **no substitue for C++**. It only provides a subset of the features. A lot of functionality is deliberately left inaccessible, such as working with [resources (TODO)](resource-management.md) or [game states](game-state.md). If you need to access them, you definitely should [use C++](Code.md).
...
The [Visual Script Class Asset](visual-script-class-asset.md) may also be used to write custom [state machine states](state-machine-asset.md#state-types). In this case the script code is executed by a state machine whenever the state is active.
