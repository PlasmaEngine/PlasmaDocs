...
For a proper game, it is better to use an [input component](input-component.md) to forward input state to [custom code](Code.md) and then decide their which animation shall get played. Then you can forward that state to the animation controller, through a [blackboard](blackboards.md). The animation controller itself would retrieve what it shall do through the [blackboard nodes](anim-nodes-blackboard.md).
