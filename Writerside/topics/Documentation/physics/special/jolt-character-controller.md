...
The [Testing Chambers sample](testing-chambers.md) contains a [prefab](prefabs-overview.md) called **Player.plPrefab**, which demonstrates how to build your own player object. The top level node contains a *Default Character Controller* component. You could replace this with a custom character controller component, to test out entirely different movement behavior.

Note that the player object also uses an [input component](input-component.md) to funnel input into a [script](TypeScript.md), which implements high level game logic, like weapon selection.
