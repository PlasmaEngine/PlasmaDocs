...
These ropes are not able to pull on another object and thus can't be used as a gameplay element. Use the [rope component](jolt-rope-component.md) for such use cases.
...
A rope requires two anchor points between which it hangs. One anchor point is the rope object position itself, for the other one typically uses a dummy game object. The `Anchor` [object reference](object-references.md) is used to select which one to use.
...
[Run the scene](run-scene.md) to see the final shape and behavior.
...
* `Anchor`: A [reference](object-references.md) to an object whose position determines where the rope ends.
