...
The image above shows some objects lit only with the skylight component. Here the scene uses a [skybox](sky.md) with blue sky, which is why the objects appear with a slightly blue tint.

## Skylight vs. Ambient Light Component

The [ambient light component](ambient-light-component.md) uses a fixed color for lighting objects. Although you could animate those colors over time, for instance using a [property animation (TODO)](Property-Animation.md), it is not possible to be directional. Using the skylight component you can have a bit of directional lighting.
...
Therefore, the skylight component requires you to select those few objects and [tag them](tags.md), such that the update of the skylight only includes those objects.
...
> **Important:**
>
> When you insert the skylight into the scene, it will override existing ambient lighting, and your scene may turn black. That's because no object in the scene is yet contributing to the skylight. You need to add the proper tag on your [sky](sky.md) to get the desired illumination.
...
* `IncludeTags`, `ExcludeTags`: These [tags](tags.md) define which objects in the scene are used to compute the skylight. Make sure that the object that renders your [sky](sky.md) has this include tag set. This is the same mechanism as used in the [camera component](camera-component.md).
