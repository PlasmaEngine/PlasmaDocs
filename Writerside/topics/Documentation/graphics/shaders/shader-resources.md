...
CONSTANT_BUFFER(plTestPositions, 1) // Slot 1, bind group 0 (default)
{
  ...
};

CONSTANT_BUFFER2(plTestPositions, SLOT_AUTO, BG_MATERIAL) // Slot Auto, bind group 2
{
  ...
};
```
...
```cpp
// By default, GetBindGroup uses `PL_GAL_BIND_GROUP_FRAME`.
plBindGroupBuilder& bindGroup = plRenderContext::GetDefaultInstance()->GetBindGroup();
plBindGroupBuilder& bindGroupRenderPass = plRenderContext::GetDefaultInstance()->GetBindGroup(PL_GAL_BIND_GROUP_RENDER_PASS);
plBindGroupBuilder& bindGroupMaterial = plRenderContext::GetDefaultInstance()->GetBindGroup(PL_GAL_BIND_GROUP_MATERIAL);
plBindGroupBuilder& bindGroupDraw = plRenderContext::GetDefaultInstance()->GetBindGroup(PL_GAL_BIND_GROUP_DRAW_CALL);
```
...
```cpp
plGALTextureRange textureRange;
...
bindGroup.BindTexture("ReflectionOutput", pFilteredSpecularOutput->m_TextureHandle, {m_uiSpecularOutputIndex * 6, 6, uiMipMapIndex, PL_GAL_ALL_MIP_LEVELS});
```
...
3. `ByteAddressBuffer` in just an array of bytes. A raw buffer needs to be bound to the resource. With HLSL 5.1, you can cast any offset of the buffer into a struct. Maps to `plGALShaderResourceType::ByteAddressBuffer` in C++.
